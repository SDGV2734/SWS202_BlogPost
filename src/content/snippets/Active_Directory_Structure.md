...
slug: Active_Directory_Structure
description: Manageing the different users in Linux system   
image: ../images/snippets/Active_Directory_Structure.png
---
...
| domain computer  | domain users |
| --- | --- |
| domain group information  | organization units (OUs) |
| default domain policy  | functional domain levels |
| password policy  | group policy objects (GPOs) |
| domain trusts  | access control list (ACLs) |
...
Active Directory is arranged in a hierarchical tree structure, with a forest at the top containing one or more domains, which can themselves have nested subdomains. A forest is the security boundary within which all objects are under administrative control. A forest may contain multiple domains, and a domain may include further child or sub-domains. A domain is a structure within which contained objects (users, computers, and groups) are accessible. It has many built-in Organizational Units (OUs), such as Domain Controllers, Users, Computers, and new OUs can be created as
required. OUs may contain objects and sub-OUs, allowing for the assignment of different group policies.
...
![image.png](../images/snippets/ActiveDirectoryStructure/image.png)

The graphic below shows two forests, INLANEFREIGHT.LOCAL and FREIGHTLOGISTICS.LOCAL. The two-way arrow represents a bidirectional trust between the two forests, meaning that users in INLANEFREIGHT.LOCAL can access resources in FREIGHTLOGISTICS.LOCAL and vice versa. We can
...
![image.png](../images/snippets/ActiveDirectoryStructure/image1.png)